...
float RatioB=0.500; //How many anticonformists in the array
boolean  AntyConfoSelfRemoving=true;//Removing the self of anticonformist from local majority
float Noise=0; //some noise as a ratio of -MaxStrengh..MaxStrengh
float Bias=0;  //Fore future testing. Positive BIAS promote "ones", negative promote "zeros" (scaled by MaxStrenght!)

int   N=75;       //array side

//For future use:
float MaxStrengh=100;//Currently fixed. It cannot be 0 or negative!
...
class TheModel
{
  ...
  boolean B[][];// new boolean[N][N]; //Anticonformity
  
  TheModel()
  {
      A = new int[N][N];     //Attitudes  
      P = new float[N][N];   //Strengh or "power"
      B = new boolean[N][N]; //Anticonformity
  }
...
void DoModelInitialisation()
{
  ...
  println("MODEL INITIALISATION... " 
    ...
    + RatioB +'\t' //How many anticonformists in the array
...
void DoMonteCarloStep()
{
   ...
   for(int a=0;a<N*N;a++) //as many times as number of cells 
   {
     ...
     if(AntyConfoSelfRemoving && B[i][j]) //Removing the self support of anticonformist from local majority
     {
         support--;
     }  
