...
////////////////////////////////////////////////////////
TheModel   MyModel;//All model data and dynamics. Also control parameters are in file Model.pde
...
//(Control variables for most nested virtual loop)
////////////////////////////////////////////////////////
int NumberOfRepetitions=100; //How many shoud be done
...
void draw() //Running - visualization, statistics and model dynamics
{
  ...
  //We have to do it that way because there is intrinsic draw loop hardcoded into Processing program! 
  if(Running && STOPAfter<StepCounter) //What to do when particular simulation run stoped?  
  {

    if(ParameterVal >= ParameterEnd
    && CurrentRepetition >= NumberOfRepetitions ) //When there are no repetitions and parameter steps left, then stop
    {
    ...
    else
    {
        ...
        {
          ParameterVal+=ParameterStep;//We have to use "double". "Float" type is not precise enough for parameter value manipulation! 
          if(ParameterVal>ParameterEnd)
          {
